...
#define PX (vec2(1.0) / u_resolution)
...
float snoise(vec2 v2, float sizemod){
  vec3 v = vec3(v2/(9.0 + sizemod), u_time / 2.5333);
...
void main(void)
{
	...
	vec2 rposition = rotate(position, u_time/-11.0);
	vec2 tposition = rposition + vec2(cos(u_time / 13.0), sin(u_time/17.0));
...
if (bRotate==false) {
	...
	vec2 position_size = tposition * SIZE; // rotate(tposition * S, u_time / 11.0);

	tile_pos = ceil(position_size);

	vec2 sw_pos = rotate(tile_pos, (u_time * 0.05) + tile_pos.x - tile_pos.y);
	//vec2 secwave = 1.0 / abs(cos(((sw_pos * 0.15) + (u_time *1.5)) * 0.5 ));
	//secwave -= 1.0;
	...
	if ((aip.x < border_limit.x) &&
		...
		vec3 cfade = mix(c, vec3(0.33 + (nsin(u_time*3.0 + length(tile_pos)) * 0.118)), 0.66);
