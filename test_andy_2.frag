...
void main()
{
    ...
    // using an iq styled camera this u_time :)
    // ray origin
    vec3 ro = 0.7 * vec3(cos(SPEED *u_time), 0.0, sin(SPEED * u_time));
    ro.y = 01.;//cos(0.6 * u_time) * 0.3 + 0.65;
    // camera look at
    vec3 ta = vec3(0.0, 0.2, 0.0);

    // camera shake intensity
    float shake = clamp(3.0 * (.0 - length(ro.yz)), 0.3, 1.0);
    float st = mod(u_time*.1, 10.0) * 143.0;

    // build camera matrix
    vec3 ww = normalize(ta - ro + noise31(st) * shake * 0.01);
    vec3 uu = normalize(cross(ww, normalize(vec3(0.0, 1.0, 0.2 * sin(u_time*SPEED)))));
    ...
    for (int i = 0; i < 4; i ++)
    {
        ...
        if (its.x > 0.0)
        {
            ...
            if (i == 3)
            {
                float crd = 0.0;//fract(u_time * 0.2) * 50.0 - 25.0;
                float fxi = cos(vo.x * 0.2 + u_time * 1.5);//abs(crd - vo.x);
    ...
    inten *= 0.4 + (sin(u_time) * 0.5 + 0.5) * 0.6;
