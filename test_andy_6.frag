...
float plasma(vec2 p, float offset) {
    ...
    for(float i = 0.0; i < 30.0; i++) {
        float t1 = iu_time * 0.04 + offset;
        float t2 = iu_time * 0.08 + offset;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 uv = rotate(framecoords, cos(iu_time*0.054)*5.0+cos(iu_time*0.013)*10.0)*(1.0+cos(iu_time*0.67)*0.5);
    uv += vec2(cos(iu_time*0.05*2.0), sin(iu_time*0.05*3.0))*0.5;
